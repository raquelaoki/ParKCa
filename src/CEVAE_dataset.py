...
class DATA(object):
    def __init__(self, ncol,tcol, data_s, y01, replications = 1): #path_data="datasets/IHDP/csv",  replications=10
        self.data_s = data_s
        self.ncol = ncol
        self.tcol = tcol
        self.y01 = y01
    ...
    def __iter__(self):
        for i in range(self.replications):
            ...
            t, y , x = data[:,self.tcol], self.y01, np.delete(data,self.tcol,1)
            yield (x, t, y)#, (y_cf, mu_0, mu_1)

    def get_train_valid_test(self):
        for i in range(self.replications):
            data = self.data_s
            t, y , x = data[:,self.tcol], np.asmatrix(self.y01).reshape(-1,1), np.delete(data,self.tcol,1)
