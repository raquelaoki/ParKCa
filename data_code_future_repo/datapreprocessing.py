...
#TODO: Create an jupyter notebook with examples and outputs
#TODO: Create a class per dataset, return X, y, others
#TODO: for similated dataset, check on M3e2 for last modificaitions

def load_GE(filename1, filename2):
    """
    TCGA - Function 1
    Function to load gene expression and clinical data, drop not important columns
    input:
        - filename1: gene expression data all
        - filename2: clinical data all
    output:
        - gene expression and clinical information
    """
...
def read_GE(filename1, filename2):
    ...
    TCGA - Function 2
    parameters:
        filename1 and filename2: address of ge and cl data (from load_GE)
    return:
...
def sim_load_h5_to_PCA(h5_path):
    """
    semi-simulated dataset
    This function loads dataset from h5 format file, remove non-informative columns, and fit a PCA
    input:
        - path file
    output:
        - PCA coordenates
...
def sim_dataset(G0, lambdas, n_causes, n_units, randseed):
    """
    semi-simulated gwas (also being used on M3E2)
...
def sim_level1data(simulations, tc, y01, roc_name):
    ...
    for i in simulations:
        ...
        coef, coef_continuos, roc, coln = models.deconfounder_PPCA_LR(np.asmatrix(train), train.columns, y01_sim1, sim,
                                                                      15, 10)
        # Join CEVAE results
        ...
        # Create dataset
        data = pd.DataFrame(
            {'cevae': cevae['cate'], 'coef': coef, 'y_out': tc_sim1_bin, 'coef_c': coef_continuos, 'y_c': tc_sim1})
